...
export default function MapCard(props: Props) {
	...
	const imageUrl = convertFileSrc(path.join(song.songPath, song.coverFileName.slice(1, -1)));
	return (
		<div className="w-full h-32 align-middle flex bg-white bg-opacity-5 p-2 rounded-2xl hover:bg-opacity-20">
			<div className="h-full w-32 float-left">
				<Image
					src={song.coverFileName.startsWith('error') ? '/icon.png' : imageUrl}
			...
			<div className="w-full float-right relative">
				<h1 className="font-bold">
					{song.title.slice(1, -1).replace(/<\/?[^>]+(>|$)/g, '')}
				</h1>
